...
**Two pump controls (Filter Pump vs a POOL PUMP outlet):** if your Vi has an outlet assigned to the pump (e.g. `OUTLET_2` named "POOL PUMP"), HomeKit gets two controls for one physical pump. Keep control on **Filter Pump** (the Vi's filtration logic — it runs the chlorinator interlocks and timers) and hide the raw outlet via `"hiddenDevices": ["OUTLET_2"]`. This also fixes Siri matching "pool temperature" to the pump switch instead of the Pool Temperature sensor.
